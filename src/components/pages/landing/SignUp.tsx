    ...
    return (
        <div className='w-full h-full flex flex-col justify-center items-center'>
            ...
            <div className='w-[24vw] flex flex-col gap-[1rem] p-[1rem] border-[1px] border-[#282828]'>
                ...
                <button onClick={()=>{navigate('/location')}} disabled={phoneNumber.length !== 10}  className={`${phoneNumber.length!=10 && ('opacity-[0.5]')} w-full aldrich-regular bg-[#FBFBFB] p-[1rem] rounded-small`}>Create Account</button>
            </div>
            <p className='text-white aldrich-regular text-[0.8rem] mt-[1rem]'>
                Already have an account? <span className='text-[#3C4CDC] cursor-pointer' onClick={()=>{navigate('/login')}}>Sign In</span>
            </p>
        </div>
    )
}
